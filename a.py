...
dataDir = '../Faster-RCNN-TensorFlow-2/dataset/'
CK5cats = ['chapter', 'section', 'clause']

dataset_dir = './company article/'
CKimg_dir = dataset_dir + "images"
CKanno_dir = dataset_dir + "Annotations"
...
def save_annotations(dataType, filename, objs):
    ...
    img_path = dataDir + dataType + "/images/" + filename
...
def showbycv(coco, dataType, img, classes, CK5Ids):
    ...
    filepath = '%s%s/images/%s' % (dataDir, dataType, filename)
...
def get_CK5():
    ...
    for dataType in dataTypes:
        annFile = '{}{}/{}.json'.format(dataDir, dataType, dataType)
        coco = COCO(annFile)
...
def split_traintest(trainratio=1, valratio=0, testratio=0.1):
